...
**Using a single instance for a production WebSocket server**
Symptoms:
- 50,000 connections and the server runs out of memory or hits the file descriptor limit
- A new deploy disconnects every user at once
- Scale out horizontally and use a backplane for cross-instance messages

**Broadcasting by looping over sockets on a single instance**
Symptoms:
- `connections.forEach(conn => conn.socket.send(msg))` — never reaches other instances
- A user on instance B never gets a message sent from instance A
- Use a pub/sub backplane (Redis, NATS) so every instance's subscribers listen on the same channel

> 💡 Real-world example
>
> **Discord Gateway:**
> - Uses a pub/sub backplane across instances/regions so a message in a guild reaches every user connected anywhere ([Discord Developer Docs — Gateway](https://discord.com/developers/docs/topics/gateway))
>
> **Pusher:**
> - A managed WebSocket infrastructure product built to scale connections horizontally through a distributed backplane (no specific concurrent-connection figure is claimed, since no primary source confirms one)
