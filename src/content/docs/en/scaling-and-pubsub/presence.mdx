...
**Marking presence without a TTL**
Symptoms:
- A user's process crashes and their presence key stays in Redis forever
- The "online" list shows users who have actually been offline for a long time
- Always use a TTL (30-60 seconds) plus a heartbeat refresh — let expiry do the cleanup for you

**Not coalescing presence writes during a stampede**
Symptoms:
- An instance restarts and 10,000 clients reconnect at once, hammering Redis with simultaneous writes
- Redis latency spikes, and the shared store falls over
- Debounce or batch presence writes during a reconnect surge

> 💡 Real-world example
>
> **The general shape large chat/collaboration platforms use (no specific interval is claimed, since no primary source confirms one):**
> - Presence lives in a distributed store with a TTL — the client refreshes before it expires, so there's no explicit "offline" event to wait for; expiry itself does the cleanup
>
> **The pattern multiplayer editors use (the same shape Figma follows):**
> - Shows cursors and active users in a document in real time
> - The presence key expires after idle time — the cursor disappears on its own if the user closes the tab without any explicit signal
