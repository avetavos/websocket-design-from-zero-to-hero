...
Symptoms:
- Messages travel over the network unencrypted
- An attacker on the same network (café wifi) can sniff the traffic
- Always use `wss://` (WebSocket Secure) in production

**No DDoS protection on the WebSocket endpoint**
Symptoms:
- An attacker opens huge numbers of WebSocket connections from many IPs
- The server runs out of file descriptors — legitimate users can't connect
- Put Cloudflare, AWS Shield, or an nginx rate limit in front of the `/ws` endpoint

> 💡 Real-world example
>
> **Cloudflare (WebSocket support):**
> - Terminates TLS at the edge — `wss://` reaches Cloudflare, then continues over its internal network ([Cloudflare — WebSockets](https://developers.cloudflare.com/network/websockets/))
> - Ships DDoS protection built in, absorbing a connection flood before it ever reaches the origin server
>
> **Discord Gateway:**
> - Always uses `wss://` — every message is encrypted on the wire ([Discord Developer Docs — Gateway](https://discord.com/developers/docs/topics/gateway))
> - Per-connection rate limits (see the rate-limiting lesson) cap the blast radius of abuse
