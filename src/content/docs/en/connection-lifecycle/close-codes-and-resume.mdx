...
**Reconnecting on code 1000**
Symptoms:
- Reconnect logic doesn't distinguish 1000 from 1006
- A user logs out, and the app just reconnects in a loop
- Check `code === 1000` and stop reconnecting; `code === 1006` and reconnect with backoff

**Not tracking a last-seen ID — data goes missing**
Symptoms:
- Reconnecting without requesting the messages that were missed
- The user sees a gap in the real-time data
- Send `lastSeenId` on reconnect so the server can replay missed messages

> 💡 Real-world example
>
> **Discord Gateway:**
> - Uses 4xxx close codes like 4000 (Unknown error), 4007 (Invalid seq), and 4009 (Session timed out) so the app can tell whether to resume or reconnect fresh ([Discord Developer Docs — Gateway](https://discord.com/developers/docs/topics/gateway))
>
> **Slack (legacy RTM API, deprecated):**
> - Used to send a `goodbye` message before closing, so the client would know the close was intentional rather than a dropped connection
