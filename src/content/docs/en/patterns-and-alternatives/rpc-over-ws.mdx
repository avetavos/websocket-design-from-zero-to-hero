...
**No request timeout for RPC**
Symptoms:
- Sending an RPC request and waiting for a response with no time limit
- The server never responds (a crash, a bug) and the client hangs forever
- Set a timeout per request: resolve with an error if no response arrives within N seconds

**Using WebSocket RPC where HTTP REST is enough**
Symptoms:
- A one-off operation (login, create a post) goes over WebSocket RPC
- Unnecessary overhead from a persistent connection
- Use HTTP for stateless operations, WebSocket for stateful real-time ones

> 💡 Real-world example
>
> **tRPC over WebSocket:**
> - tRPC's `wsLink` provides real type-safe RPC over WebSocket — used for real-time subscriptions, while queries/mutations can still go over HTTP ([tRPC Docs — WebSockets](https://trpc.io/docs/server/websockets)) (Discord and Linear are omitted here since no primary source confirms either uses tRPC)
>
> **JSON-RPC 2.0 over WebSocket:**
> - A standard protocol for RPC over WebSocket
