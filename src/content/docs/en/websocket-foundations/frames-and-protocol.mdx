...
**Sending a large payload without fragmenting it**
Symptoms:
- Sending a 10MB JSON blob in a single frame
- The receiver has to wait for the whole frame before it can process anything — blocking
- Use fragmentation or paginate the message instead

**Using `ws://` in production**
Symptoms:
- Data travels as plaintext over the network
- A man-in-the-middle can intercept or inject data
- Always use `wss://` in production

> 💡 Real-world example
>
> **Google Meet:**
> - Uses binary frames for video/audio data over WebRTC (codecs such as VP8, Opus)
> - Uses text frames (JSON) for control messages like mute, chat, and the participant list
>
> **General principle (the same shape many multiplayer/design tools use):**
> - Send cursor position and selection state as binary frames instead of JSON to cut parsing overhead on very high-frequency events (no primary source backs a specific percentage figure here, so none is claimed)
