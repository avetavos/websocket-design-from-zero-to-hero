...
ถึงเวลาเขียน server จริงสักตัว ใน Node ไลบรารีโดยพฤตินัยคือ [`ws`](/websocket/th/server-design/) — เล็ก, เร็ว, และใกล้ชิดกับโปรโตคอล บทเรียนนี้จะพาเดินผ่านสามชิ้นส่วนหลักของตัวเอง: การสร้าง server, `connection` event, และการเรียก `message`/`send` ต่อ socket
...
>
> **Socket.IO:**
> - library ยอดนิยมสำหรับทำ real-time server บน Node.js — ครอบ raw WebSocket ด้วย room, namespace, และ automatic fallback ให้ (ตัดชื่อบริษัทลูกค้าทิ้ง เพราะ Trello เคยใช้แล้วย้ายออกเมื่อ scale เกิน ~10,000 connection ต่อ process และ Notion ไม่พบแหล่งอ้างอิงปฐมภูมิว่าใช้จริง)
>
> **uWebSockets.js:**
> - benchmark อย่างเป็นทางการวัดได้ throughput และจำนวน concurrent connection สูงกว่า Socket.IO หลายเท่าบน hardware เดียวกัน ([uNetworking/uWebSockets — benchmarks](https://github.com/uNetworking/uWebSockets/blob/master/benchmarks/README.md)) — ไม่ระบุตัวเลข "1M+" หรือชื่อ Binance เพราะไม่พบแหล่งอ้างอิงปฐมภูมิยืนยัน
