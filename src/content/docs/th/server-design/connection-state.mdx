...
description: 'การแนบ state ต่อ connection — user id, subscriptions — ให้แต่ละ socket และ clean up อย่างน่าเชื่อถือตอน close พร้อมเดโม Node ws ที่รันได้จริง'
sidebar:
  order: 3
---
...
> 💡 ตัวอย่างจากของจริง
>
> **หลักการทั่วไปที่ chat gateway ขนาดใหญ่ใช้ (ไม่ระบุ store เจาะจง เพราะไม่พบแหล่งอ้างอิงปฐมภูมิยืนยันรายละเอียด infra ภายใน):**
> - เก็บแค่ identifier ระดับ connection: userId, guild/workspace id ที่เกี่ยวข้อง, channel ที่ subscribe อยู่
> - session state ที่ต้องคงอยู่ข้าม reconnect เก็บใน external session store แยกจาก process ของ connection เอง — ไม่ผูกกับ process ที่จะหายไปเมื่อ restart
> - ข้อมูลที่หนัก (full profile, history) อยู่ใน database หลัก แล้ว query เมื่อต้องการเท่านั้น ไม่แคชไว้ใน connection object

<Callout title="state เข้า, state ออก">
แก้ identity **หนึ่งครั้ง** ตอน connect แล้วแนบ state (user id, subscriptions) เข้ากับ socket หรือ side-table `Map` mutate state เมื่อข้อความเข้ามา — และลบ **ทุก** ชิ้นส่วนของตัวเองออกตอน `close` *และ* `error` state ที่ไม่ได้ clean up คือ memory leak และเป็นต้นตอของ ghost broadcast
</Callout>
...
export const quizState = [
  ...
  },
  {
    ...
    options: [
      ...
      'ทำให้ความเร็ว connection เพิ่มเป็นสองเท่า',
    ],
    answer: 1,
  },
];
