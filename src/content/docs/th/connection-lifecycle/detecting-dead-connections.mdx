---
title: 'การตรวจจับ Dead Connection'
description: 'เปลี่ยน pong ที่หายไปให้กลายเป็นความมั่นใจว่า "อันนี้ตายแล้ว" — timeout, half-open connection, การ terminate อย่างสะอาด และเดโมสดที่ตั้งธง pong ที่หายไป'
sidebar:
  order: 3
---

import NodeRunner from '../../../../components/NodeRunner.tsx';
import Mermaid from '../../../../components/Mermaid.astro';
import Callout from '../../../../components/Callout.astro';
import Quiz from '../../../../components/Quiz.tsx';
import ProgressTracker from '../../../../components/ProgressTracker.tsx';

Heartbeat จากบทเรียนที่แล้วส่งชีพจรออกไป แต่ชีพจรที่คุณส่งนั้นไร้ค่าหากคุณไม่ *ตรวจสอบว่าคำตอบกลับมา* บทเรียนนี้ว่าด้วยการตรวจสอบนั้น: วิธีเปลี่ยน "pong ไม่มาถึง" ให้กลายเป็นการตัดสินใจที่มั่นใจว่าจะประกาศว่า connection ตายแล้วรื้อทิ้ง

## การส่งคือครึ่งหนึ่ง การคาดหวังคืออีกครึ่ง

ความพยายามแรกที่ผิดและพบบ่อยคือการส่ง ping บน timer แล้วหยุดแค่นั้น นั่นไม่ตรวจจับอะไรเลย การตรวจจับทั้งหมดเกิดขึ้นใน **ช่องว่างระหว่าง ping ที่คุณส่งกับ pong ที่คุณคาดหวัง** Logic คือ:

1. ส่ง ping จดไว้ว่าตอนนี้มี pong *ค้าง* อยู่กับคุณ
2. เริ่ม deadline — เช่น interval ถัดไปหรือไม่กี่วินาที
3. ถ้า pong ที่ตรงกันมาถึงก่อน deadline ดีแล้ว: เคลียร์หนี้ connection ยังมีชีวิต
4. ถ้า deadline ผ่านไปโดยที่หนี้ยังค้างอยู่ แสดงว่าคุณมี **missed pong** ให้ถือว่า connection ตายแล้ว

ดังนั้นตัวแปรที่มีประโยชน์ที่สุดใน client ของคุณคือสิ่งที่คล้าย ๆ `pongOutstanding` (หรือจำนวน ping ที่ยังไม่ได้รับคำตอบ) พอค่านี้ข้ามเกณฑ์เมื่อไหร่ก็ลงมือได้ client ใน production ส่วนใหญ่ยอมให้ miss สักหนึ่งถึงสองครั้งก่อนประกาศการตาย — pong ที่หายไปครั้งเดียวอาจเป็นแค่อาการสะดุดชั่วขณะ แต่สามครั้งติดกันคือศพ

## Half-open connection — เหตุผลว่าทำไมเรื่องนี้ถึงยาก

ตัวร้ายเบื้องหลังทั้งหมดนี้คือ connection แบบ **half-open** connection จะเป็น half-open เมื่อฝ่ายหนึ่งยังเชื่อว่าต่ออยู่ ส่วนอีกฝ่ายหายไปแล้วอย่างเงียบ ๆ ลองนึกภาพการโทรศัพท์ที่โทรศัพท์ของอีกฝ่ายดับไปกลางประโยค: คุณยังถือหูฟังอยู่ พูดอยู่ ไม่ได้ยินอะไรเลย โดยไม่มีเสียงสัญญาณบอกว่าสายตัดไปแล้ว

สิ่งนี้เกิดขึ้นตลอดเวลาในเน็ตเวิร์กจริง:

- ฝาแลปท็อปปิด OS แช่แข็ง socket โดยไม่มีการ close อย่างสะอาด
- อุปกรณ์มือถือสลับจาก Wi-Fi ไปเป็นเซลลูลาร์ ทิ้ง socket เก่าไป
- NAT หรือ load balancer ขับ connection ที่ idle ออกจากตารางอย่างเงียบ ๆ

ในทุกกรณี **ไม่มี event `close` ยิงขึ้น** บนฝ่ายที่ยังอยู่รอด เพราะไม่มี FIN packet มาถึงเลยสักตัว `WebSocket` ของคุณจึงยังบอกว่า OPEN วิธี *เดียว* ที่จะรู้ความจริงคือส่งอะไรบางอย่างออกไปแล้วดูว่าไม่มีอะไรกลับมา — ซึ่งนั่นคือสิ่งที่ heartbeat บวก deadline มอบให้คุณพอดี

## จาก missed pong สู่การ termination

เมื่อตัดสินใจแล้วว่า connection ตาย ก็ไม่ต้องสุภาพ การ `close()` อย่างสะอาดจะพยายามทำ handshake กับ peer ที่ *หายไปแล้ว* — ไม่มีใครให้ handshake ด้วย จึงมีสิทธิ์ค้าง ให้ **terminate** แทน: ทิ้ง socket ที่ตายแล้วทันทีแล้วไป reconnect ตรง ๆ เบราว์เซอร์เปิดเผย `close()` ส่วน library `ws` ของ Node เพิ่ม `terminate()` มาเพื่อกรณีโหด ๆ แบบนี้โดยเฉพาะ Flow การตัดสินใจ:

<Mermaid title="การตัดสินใจว่า connection ตายแล้ว" code={detectDiagram} />

export const detectDiagram = `stateDiagram-v2
  [*] --> Healthy
  Healthy --> Waiting: send ping, pong now owed
  Waiting --> Healthy: pong arrived in time
  Waiting --> Suspect: deadline passed, pong missed
  Suspect --> Healthy: a later pong arrived
  Suspect --> Dead: misses exceeded threshold
  Dead --> Terminate: abandon socket, do not handshake
  Terminate --> [*]: hand off to reconnect`;

## ดู missed pong ถูกตั้งธง

เดโมด้านล่างใช้ socket ในหน้าที่มี `WebSocket` API จริง mock นี้ตอบ ping สองสามตัวแรกตามปกติ — แล้วก็เงียบไป จำลอง connection ที่ตายไปแล้วโดยไม่ยิง `onclose` (half-open connection) client รัน heartbeat ที่มี deadline และตั้งธงว่า connection ตายในวินาทีที่ pong เกินกำหนด

export const deadDetectCode = `(function () {
  // In-page socket with the real WebSocket API. This mock answers the first
  // two pings, then goes SILENT to imitate a half-open connection: the socket
  // still says OPEN but no pong ever returns and no onclose ever fires.
  // In a real app: const ws = new WebSocket('wss://your-server').
  class FlakySocket {
    constructor() {
      this.readyState = 0;
      this.answered = 0;
      setTimeout(() => { this.readyState = 1; if (this.onopen) this.onopen({}); }, 30);
    }
    send(data) {
      const msg = JSON.parse(data);
      if (msg.type !== 'ping') return;
      this.answered += 1;
      if (this.answered <= 2) {
        setTimeout(() => {
          if (this.onmessage) this.onmessage({ data: JSON.stringify({ type: 'pong', seq: msg.seq }) });
        }, 30);
      }
      // 3rd ping onward: silence. No pong, no close. The connection is a corpse.
    }
    close(code) { this.readyState = 3; if (this.onclose) this.onclose({ code: code || 1000 }); }
    terminate() { this.readyState = 3; /* no handshake — just gone */ }
  }

  const PING_MS = 600;
  const ws = new FlakySocket();
  let seq = 0;
  let pongOutstanding = 0;
  let timer = null;

  ws.onopen = () => {
    console.log('open — heartbeat with deadline started');
    timer = setInterval(() => {
      if (pongOutstanding >= 1) {
        console.log('!! missed pong — pongs outstanding: ' + pongOutstanding);
        if (pongOutstanding >= 2) {
          console.log('connection declared DEAD — terminating (no clean handshake)');
          clearInterval(timer);
          ws.terminate();
          console.log('socket abandoned — hand off to reconnect logic');
          return;
        }
      }
      seq += 1;
      pongOutstanding += 1;
      console.log('-> ping #' + seq + ' (pong now owed)');
      ws.send(JSON.stringify({ type: 'ping', seq: seq }));
    }, PING_MS);
  };

  ws.onmessage = (event) => {
    const msg = JSON.parse(event.data);
    if (msg.type === 'pong') {
      pongOutstanding = Math.max(0, pongOutstanding - 1);
      console.log('<- pong #' + msg.seq + ' — debt cleared, connection healthy');
    }
  };
  ws.onclose = (event) => console.log('closed, code', event.code);
})();`;

<NodeRunner client:visible code={deadDetectCode} />

คุณจะเห็น ping/pong ที่แข็งแรงสองคู่ จากนั้น mock ก็เงียบไป client ส่งต่อไปเรื่อย ๆ จำนวน pong ที่ไม่ได้รับคำตอบไต่สูงขึ้น พอข้ามเกณฑ์ client ก็ประกาศว่า connection ตายแล้ว *terminate* ทิ้ง — โดยไม่ต้องรอ `onclose` ที่ไม่มีวันมา นั่นคือการตรวจจับ: คุณหยุดเชื่อความเงียบและลงมือกับคำตอบที่หายไป

| Detection Method | ความแม่นยำ | Overhead |
|---|---|---|
| Heartbeat + missed pong counter | สูง — ตรวจจับ half-open ได้ | ส่ง ping ทุก interval |
| OS TCP keepalive | ต่ำ — อาจใช้เวลาชั่วโมง | ไม่มี application code |
| Connection timeout บน server | ปานกลาง | server ต้องตรวจ idle time |

## ข้อผิดพลาดที่พบบ่อย

**รอ onclose Event สำหรับ Dead Connection**
อาการ:
- half-open connection ไม่ยิง `onclose` — รอตลอดไป
- server ถือ zombie connection โดยไม่รู้ตัว
- ใช้ heartbeat + missed pong เพื่อตรวจจับ ไม่ต้องรอ event

**Threshold สูงเกินไป — ตรวจจับช้า**
อาการ:
- threshold 10 missed pong — รอนานเกินก่อนประกาศว่าตาย
- UX แย่ — user รู้สึกว่า app ค้าง
- threshold 2-3 missed pong เหมาะสมสำหรับส่วนใหญ่

> 💡 ตัวอย่างจากของจริง
>
> **Discord Gateway:**
> - ถ้าไม่ได้รับ Heartbeat ACK ก่อนถึงรอบ heartbeat ถัดไป ให้ถือว่า connection "zombied" แล้ว terminate เพื่อ reconnect ทันที ([Discord Developer Docs — Gateway](https://discord.com/developers/docs/topics/gateway))
> - ไม่รอ TCP timeout
>
> **Cloudflare Durable Objects:**
> - runtime จะ hibernate Durable Object หลังไม่มี event เข้ามาประมาณ 10 วินาที (ไม่ใช่ 60 วินาทีตามที่เข้าใจกันทั่วไป) — client ที่ใช้ Hibernation WebSocket API ยังต่อ connection ค้างไว้ได้แม้ object จะ sleep ไปแล้ว ([Cloudflare Durable Objects — WebSockets](https://developers.cloudflare.com/durable-objects/best-practices/websockets/))

| Dead Connection Scenario | สาเหตุ | Detection Method |
|---|---|---|
| Client network drop | wifi ขาด, mobile network เปลี่ยน | Heartbeat timeout |
| Firewall/NAT timeout | idle connection ถูก reset โดยไม่แจ้ง | Heartbeat ไม่ได้ pong |
| Process crash | browser tab close, app kill | OS ส่ง TCP RST — `onclose` fire |
| Server-side timeout | server close idle connection | Close frame opcode |

## ข้อผิดพลาดที่พบบ่อยเพิ่มเติม

**เชื่อ TCP Connected ว่า Application Connected**
อาการ:
- TCP connection ยังอยู่แต่ data ไม่ผ่าน (half-open connection)
- server ส่ง message ไปแต่ client ไม่ได้รับ — ไม่มี error
- heartbeat ที่ application level จำเป็น — TCP status ไม่เพียงพอ

**Zombie Connection ที่ Server ไม่รู้**
อาการ:
- client disconnect แบบ ungraceful (network cut, process kill)
- server ยัง track connection อยู่ใน map — memory leak
- ตั้ง heartbeat timeout: ถ้าไม่มี pong ใน N วินาที → remove และ close

> 💡 ตัวอย่างจากของจริง
>
> **AWS API Gateway WebSocket:**
> - idle connection timeout ตายตัวที่ 10 นาที (เพิ่มไม่ได้) และ connection lifetime สูงสุด 2 ชั่วโมง ([AWS Docs — WebSocket API quotas](https://docs.aws.amazon.com/apigateway/latest/developerguide/apigateway-execution-service-websocket-limits-table.html))
> - แอปต้องส่ง heartbeat ถี่กว่า 10 นาทีเสมอถ้าต้องการ connection ค้างนาน
>
> **nginx WebSocket proxy:**
> - `proxy_read_timeout` default คือ 60 วินาที — ต้องตั้งเป็น `proxy_read_timeout 3600s` (หรือมากกว่า) สำหรับ WebSocket ที่เปิดค้างนาน ([nginx — proxy_read_timeout](https://www.getpagespeed.com/server-setup/nginx/nginx-timeout))
> - ถ้าไม่ตั้ง → nginx ปิด connection idle ทุก 60 วินาที (ตรงกับที่ระบุไว้ข้างต้น)

<Callout title="การตรวจจับในหนึ่งบรรทัด">
Connection ตายเมื่อ pong ที่ค้างอยู่ไม่มาถึงก่อน deadline นับจำนวนการ miss ข้ามเกณฑ์เล็ก ๆ แล้ว **terminate** แทนการ close อย่างสุภาพ — เพราะ half-open peer หายไปแล้วและไม่เหลือใครให้ handshake ด้วย
</Callout>

export const quizDetect = [
  {
    q: 'อะไรคือสิ่งที่ตรวจจับ dead connection ได้จริง ๆ?',
    options: [
      'แค่ส่ง ping ตาม timer ก็เพียงพอแล้ว',
      'ช่องว่างระหว่าง ping ที่ส่งไป กับ pong ที่ไม่เคยมาถึงก่อน deadline',
      'การรอ event onclose ของเบราว์เซอร์ ซึ่งยิงเสมอ',
      'การอ่าน property readyState ครั้งเดียวตอนเริ่มต้น',
    ],
    answer: 1,
  },
  {
    q: 'Half-open connection คืออะไร?',
    options: [
      'Connection ที่ถูกเข้ารหัสในทิศทางเดียวเท่านั้น',
      'ฝ่ายหนึ่งยังเชื่อว่าต่ออยู่ ส่วนอีกฝ่ายหายไปแล้วอย่างเงียบ ๆ โดยไม่มี close event ยิงเลย',
      'Connection ที่ยังอยู่ในเฟส handshake',
      'Connection ที่ถูกจำกัดไว้ที่ครึ่งหนึ่งของ bandwidth ปกติ',
    ],
    answer: 1,
  },
  {
    q: 'ทำไมต้อง terminate แทนการ close อย่างสะอาดสำหรับ connection ที่คุณประกาศว่าตายแล้ว?',
    options: [
      'การ terminate ถูกเข้ารหัส ส่วนการ close ไม่ถูกเข้ารหัส',
      'การ close อย่างสะอาดพยายาม handshake กับ peer ที่หายไปแล้วและอาจค้าง ดังนั้นคุณจึงละทิ้ง socket แทน',
      'Protocol ห้ามการ close dead connection',
      'ไม่มีความแตกต่างระหว่างสองอย่างนี้',
    ],
    answer: 1,
  },
];

<Quiz client:visible id="connection-lifecycle/detecting-dead-connections" questions={quizDetect} />

<ProgressTracker client:visible id="connection-lifecycle/detecting-dead-connections" />
