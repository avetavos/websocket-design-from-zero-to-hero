...
> - server บังคับตัด connection ทุก 24 ชั่วโมง (ส่ง `serverShutdown` เตือนล่วงหน้า) และปิด connection ถ้าไม่ตอบ pong ภายใน 1 นาทีหลัง ping ทุก 20 วินาที — client ต้องมี reconnect logic พร้อมเสมอ ไม่ใช่แค่ตอน error ([Binance API Docs — WebSocket Streams](https://developers.binance.com/docs/binance-spot-api-docs/web-socket-streams))
>
> **หลักการที่ยอมรับกันทั่วไป:**
> - ใช้ exponential backoff ร่วมกับ jitter แบบสุ่ม เพื่อไม่ให้ client จำนวนมาก retry พร้อมกันเป็นคลื่น ("thundering herd") — ไม่ระบุตัวเลข schedule เจาะจงของผู้ให้บริการรายใดรายหนึ่ง เพราะไม่พบแหล่งอ้างอิงปฐมภูมิยืนยัน ดูหลักการเต็มได้ที่ [AWS Architecture Blog — Exponential Backoff and Jitter](https://aws.amazon.com/blogs/architecture/exponential-backoff-and-jitter/)
...
export const quizBackoff = [
  {
    q: 'ทำไมการ reconnect ทันทีใน loop ที่กระชั้นถึงอันตราย?',
    options: [
      'กินแบตเตอรี่บน server มากเกินไป',
      ...
      'เบราว์เซอร์ปฏิเสธการ reconnect ทันทีเสมอ',
    ],
    answer: 1,
  },
  {
    ...
    options: [
      'เข้ารหัสความพยายาม reconnect',
      'de-synchronize client เพื่อไม่ให้ retry พร้อมกันเป็นคลื่นเดียว',
      'ทำให้ดีเลย์โตเร็วขึ้น',
      'ทำให้ไม่ต้องมี cap อีกต่อไป',
    ],
    answer: 1,
  },
  {
    ...
    options: [
      ...
      'เพื่อให้การ drop ครั้งถัดไปเริ่ม backoff จากดีเลย์สั้น ๆ อีกครั้ง แทนที่จะเริ่มจากดีเลย์มหึมาที่ค้างไว้',
      'เพราะไม่อย่างนั้นตัวนับจะ overflow',
      'ไม่ควรรีเซ็ตเลย',
    ],
    answer: 1,
  },
