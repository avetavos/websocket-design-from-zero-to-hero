...
>
> **Discord Gateway:**
> - client ส่ง `{ "op": 1, "d": <last sequence> }` (Heartbeat opcode) ตาม interval ที่ server กำหนดตอน Hello
> - server ตอบ `{ "op": 11 }` (Heartbeat ACK) — ถ้าไม่มี ACK มาก่อน heartbeat รอบถัดไป ให้ถือว่า connection "zombied" แล้ว reconnect ([Discord Developer Docs — Gateway](https://discord.com/developers/docs/topics/gateway))
>
> **Slack (Legacy RTM API, deprecated):**
> - เคยใช้ ping/pong เป็น application-level heartbeat เพื่อตรวจจับ connection ที่ตายไปเงียบ ๆ ก่อน reconnect อัตโนมัติ
