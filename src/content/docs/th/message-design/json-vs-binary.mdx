...
> - ใช้ JSON เป็น format หลักของ market data stream ทั้งหมด รวมถึง subscription management ([Binance API Docs — WebSocket Streams](https://developers.binance.com/docs/binance-spot-api-docs/web-socket-streams)) — ไม่พบแหล่งอ้างอิงปฐมภูมิที่ยืนยันตัวเลข "ลด payload 60%" จึงตัดตัวเลขนี้ทิ้ง
>
> **หลักการทั่วไป (แนวเดียวกับ multiplayer/design tool หลายเจ้า):**
> - ใช้ binary format สำหรับ operation ที่ยิงถี่และมีโครงสร้างคงที่ (position, color, stroke) เพื่อลด parsing overhead, ใช้ JSON สำหรับ metadata และ configuration ที่เปลี่ยนรูปแบบบ่อยกว่า
...
export const quizJsonBinary = [
  {
    ...
    options: [
      'เล็กกว่า binary บน wire เสมอ',
      'อ่านออก ใช้ได้ทุกที่ และไม่ต้องมี tooling หรือขั้น build',
      'WebSocket ส่ง binary frame ไม่ได้',
      'parse เร็วกว่า binary format ทุกตัว',
    ],
    answer: 1,
  },
  {
    q: 'เมื่อไรที่ format แบบ binary ที่กระชับจึงคุ้มต้นทุนชัดเจนที่สุด?',
    options: [
      'สำหรับ message ตั้งค่าที่ใหญ่และไม่ค่อยเกิดเพียงไม่กี่อัน',
      'สำหรับ stream ที่เล็ก ถี่ ปริมาณสูง ซึ่ง bytes และเวลา parse ต่อ message สะสมเพิ่มขึ้น',
  ...
  {
    q: 'ใน code MessagePack ทำไมต้องตั้ง ws.binaryType = "arraybuffer"?',
    options: [
      'เพื่อบีบอัด JSON อัตโนมัติ',
      'เพื่อให้ binary message มาถึงในรูป ArrayBuffer ที่ decoder อ่านได้ แทนที่จะเป็น Blob',
      'สลับ connection ไปยัง port อื่น',
      'จำเป็นสำหรับ JSON message ทุกอันด้วย',
    ],
    answer: 1,
  },
];
