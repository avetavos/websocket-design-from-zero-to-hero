...
เบราว์เซอร์ให้ class มาหนึ่งตัว สร้างขึ้นมาเพื่อเริ่มที่ `CONNECTING` ตอบสนองผ่านสี่ event ตรวจ `readyState` เพื่อดูสี่สถานะ แล้วเรียก `send()` / `close()` ทุกอย่างในโมดูลนี้แขวนอยู่บนสมาชิกไม่กี่ตัวนั้น
</Callout>
...
export const quizIndex = [
  ...
  },
  {
    ...
    options: [
      'กลับไปเป็น OPEN ได้ถ้า peer เชื่อมต่อใหม่',
      'ปิดอย่างถาวร ถ้าจะเชื่อมต่อใหม่ต้องสร้าง object WebSocket ตัวใหม่',
      'ลองเชื่อมต่อใหม่อัตโนมัติไปเรื่อย ๆ',
      'ย้อนกลับไปที่ CONNECTING',
    ],
    answer: 1,
  },
];
