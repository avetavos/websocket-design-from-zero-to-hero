...
<Mermaid title="การส่งและ message event ที่ตามมา เรียงตามลำดับ" code={ioDiagram} />
...
> 💡 ตัวอย่างจากของจริง
>
> **หลักการทั่วไปของแอปแชท (แนวเดียวกับที่ Slack เคยใช้ใน Legacy RTM API):**
> - รับ message เป็น JSON เช่น `{ "type": "message", "text": "Hello", "channel": "C1234" }` แล้ว dispatch ตาม `type` field — event-driven message handling
>
> **Google Docs:**
> - ใช้ Operational Transformation (OT) — ส่ง operation ไปยัง server แล้วรับ update กลับมา apply ต่อ document state บนเครื่อง client ([Wikipedia — Operational transformation](https://en.wikipedia.org/wiki/Operational_transformation))
...
export const quizSendRecv = [
  ...
  },
  {
    ...
    options: [
      ...
      'เปิดการเชื่อมต่อให้เร็วขึ้น',
      'ไม่เกิดอะไรเลย — ปลอดภัยเสมอ',
    ],
    answer: 1,
  },
  {
